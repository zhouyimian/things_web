...
<div class="page-container">
    <div class="container">
        <div class="row">
            <div class="tagcloud" style="margin-left: 28px">
                ...
                <c:forEach items="${ArticlepageQuery.items}" var="item">
                    <article class=" type-post  format-standard hentry clearfix">
                        ...
                        <p>
                            ...
                            <a class="readmore-link" href="<c:url value='/ArticleController/findArticleByAid.action?aid=${item.aid}'/>">Read more</a>
                        </p>
                    </article>
                </c:forEach>
                <div id="pagination">
                    <c:if test="${ArticlepageQuery.currentPage!=1}">
                        <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=1'/>" class="btn active">首页</a>
                        <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=${ArticlepageQuery.currentPage-1}'/>" class="btn active">上一页</a>
                    </c:if>
                    <c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==ArticlepageQuery.currentPage}">
                                <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=${i}'/>" style="background: #575bd1" class="btn active">${i}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=${i}'/>" class="btn active">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < ArticlepageQuery.totalPage }">
                    ...
                    <c:if test="${ArticlepageQuery.currentPage!=ArticlepageQuery.totalPage}">
                        <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=${ArticlepageQuery.currentPage+1}'/>" class="btn active">下一页</a>
                        <a href="<c:url value='/PageQueryController/loadArticlepage.action?ArticlepageQuery=${ArticlepageQuery.totalPage}'/>" class="btn active">尾页</a>
                    </c:if>
                </div>
