            ...
            <div class="span8 main-listing">
                <c:forEach items="${ResourcepageQuery.items}" var="item">
                    <article class=" type-post  format-standard hentry clearfix">
                        ...
                        <p>
                            <div style="text-overflow:ellipsis;white-space:nowrap;overflow:hidden">${item.resource_article}</div>
                            <a class="readmore-link" href="<c:url value='/ResourceController/findResourceByRid.action?rid=${item.rid}'/>">Read more</a>
                        </p>
                    </article>
                </c:forEach>
                <div id="pagination">
                    <c:if test="${ResourcepageQuery.currentPage!=1}">
                        <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=1'/>" class="btn active">首页</a>
                        <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=${ResourcepageQuery.currentPage-1}'/>" class="btn active">上一页</a>
                    </c:if>
                    <c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==ResourcepageQuery.currentPage}">
                                <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=${i}'/>" style="background: #575bd1" class="btn active">${i}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=${i}'/>" class="btn active">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < ResourcepageQuery.totalPage }">
                    ...
                    <c:if test="${ResourcepageQuery.currentPage!=ResourcepageQuery.totalPage}">
                        <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=${ResourcepageQuery.currentPage+1}'/>" class="btn active">下一页</a>
                        <a href="<c:url value='/PageQueryController/loadResourcepage.action?ResourcepageQuery=${ResourcepageQuery.totalPage}'/>" class="btn active">尾页</a>
                    </c:if>
                </div>
            </div>
            <!-- end of page content -->
