    ...
    <div class="container">
        <div class="row">
            ...
            <div class="span8 main-listing">
                <c:forEach items="${search_resource.items}" var="item">
                ...
                <div id="pagination">
                    <c:if test="${search_resource.currentPage!=1}">
                        <a href="<c:url value='/SolrController/solrSearchResource.action?page=1'/>" class="btn active">首页</a>
                        <a href="<c:url value='/SolrController/solrSearchResource.action?page=${search_resource.currentPage-1}'/>" class="btn active">上一页</a>
                    </c:if>
                    <c:choose>
                        <c:when test="${search_resource.totalPage<=6}">
                            <c:set var="begin" value="1" />
                            <c:set var="end" value="${search_resource.totalPage}" />
                        </c:when>
                        <c:otherwise>
                            <c:set var="begin" value="${search_resource.currentPage-2 }"/>
                            <c:set var="end" value="${search_resource.currentPage + 3}"/>
                            <c:if test="${begin < 1 }">
                                <c:set var="begin" value="1"/>
                                <c:set var="end" value="6"/>
                            </c:if>
                            <c:if test="${end > search_resource.totalPage }">
                                <c:set var="begin" value="${search_resource.totalPage-5 }"/>
                                <c:set var="end" value="${search_resource.totalPage}"/>
                            </c:if>
                        </c:otherwise>
                    </c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==search_resource.currentPage}">
                                <a href="<c:url value='/SolrController/solrSearchResource.action?page=${i}'/>" style="background: #575bd1" class="btn active">${i}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="<c:url value='/SolrController/solrSearchResource.action?page=${i}'/>" class="btn active">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < search_resource.totalPage }">
                    ...
                    <c:if test="${search_resource.currentPage!=search_resource.totalPage}">
                        <a href="<c:url value='/SolrController/solrSearchResource.action?page=${search_resource.currentPage+1}'/>" class="btn active">下一页</a>
                        <a href="<c:url value='/SolrController/solrSearchResource.action?page=${search_resource.totalPage}'/>" class="btn active">尾页</a>
                    </c:if>
                </div>
