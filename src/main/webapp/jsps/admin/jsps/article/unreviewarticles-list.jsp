            ...
            <div class="pagination">
                <ul>
                    <c:if test="${ArticlepageQuery.currentPage!=1}">
                        <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=1'/>">首页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=${ArticlepageQuery.currentPage-1}'/>">上一页</a></li>
                    </c:if>
                    <c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==ArticlepageQuery.currentPage}">
                                <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=${i}'/>" style="background: #575bd1" class="btn active">${i}</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=${i}'/>" class="btn active">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < ArticlepageQuery.totalPage }">
                    ...
                    <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=${ArticlepageQuery.currentPage+1}'/>">下一页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getArticleListByState.action?state=${state}&ArticlepageQuery=${ArticlepageQuery.totalPage}'/>">尾页</a></li>
                    </c:if>
                </ul>
            </div>
