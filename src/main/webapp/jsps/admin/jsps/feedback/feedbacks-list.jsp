                        ...
                        <tr>
                            ...
                            <c:choose>
                                <c:when test="${item.state==0}">
                                    <td><a href="<c:url value='/FeedBackController/getFeedbackByFid.action?fid=${item.fid}' />"  style="color: red">未处理</a></td>
                                </c:when>
                                <c:when test="${item.state==1}">
                                    <td><a href="<c:url value='/FeedBackController/getFeedbackByFid.action?fid=${item.fid}' />">已处理</a></td>
                                </c:when>
                            </c:choose>
                        </tr>
                    </c:forEach>
            ...
            <div class="pagination">
                <ul>
                    <c:if test="${FeedbackpageQuery.currentPage!=1}">
                        <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=1'/>">首页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=${FeedbackpageQuery.currentPage-1}'/>">上一页</a></li>
                    </c:if>
                    <c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==FeedbackpageQuery.currentPage}">
                                <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=${i}'/>" style="background: #575bd1" class="btn active">${i}</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=${i}'/>" class="btn active">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < FeedbackpageQuery.totalPage }">
                    ...
                    <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=${FeedbackpageQuery.currentPage+1}'/>">下一页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getFeedbackList.action?FeedbackpageQuery=${FeedbackpageQuery.totalPage}'/>">尾页</a></li>
                    </c:if>
                </ul>
            </div>
