...
<div class="content">
    ...
    <div class="header">
        <h1 class="page-title">未审核资源</h1>
    </div>

    <ul class="breadcrumb">
        <li class="active">未审核资源</li>
    </ul>
            ...
            <div class="pagination">
                <ul>
                    <c:if test="${ResourcepageQuery.currentPage!=1}">
                        <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=1'/>">首页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=${ResourcepageQuery.currentPage-1}'/>">上一页</a></li>
                    </c:if>
                    <c:choose>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        <c:choose>
                            <c:when test="${i==ResourcepageQuery.currentPage}">
                                <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=${i}'/>" style="background: #575bd1" class="btn active">${i}</a></li>
                            </c:when>
                            <c:otherwise>
                                <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=${i}'/>" class="btn active">${i}</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:if test="${end < ResourcepageQuery.totalPage }">
                    ...
                    <c:if test="${ResourcepageQuery.currentPage!=ResourcepageQuery.totalPage}">
                        <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=${ResourcepageQuery.currentPage+1}'/>">下一页</a></li>
                        <li><a href="<c:url value='/PageQueryController/getResourceListByState.action?state=0&ResourcepageQuery=${ResourcepageQuery.totalPage}'/>">尾页</a></li>
                    </c:if>
                </ul>
            </div>
